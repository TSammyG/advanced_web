...
class Post
{
     ...
     * @param  string $user_to The user who created the post
     * @return void
     */
    public function submitPost($body, $user_to)
    {
        ...
        if ($check_empty != "") {
            ...
            $num_posts = $this->_user_obj->getNumPosts();
            $num_posts++;
            $query_string = <<<EOQ
            UPDATE users 
    ...
    public function loadPostsFriends($data, $limit)
    {
        ...
        ORDER BY id DESC
        EOQ;
        $data_query = mysqli_query($this->_con, $query_string);
        ...
        if (mysqli_num_rows($data_query) > 0) {
            ...
            while ($row = mysqli_fetch_array($data_query)) {
                ...
                FROM users 
                    WHERE username='$added_by'
                EOQ;
                $user_details_query = mysqli_query($this->_con, $query_string);
